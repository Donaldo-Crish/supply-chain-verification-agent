...
def trace_journey(product_id):
    ...
    journey = [
        {"stage": "Manufacturer",    "location": mfg_loc,           "date": base_date.strftime('%Y-%m-%d'),                    "verified": True},
        {"stage": "Quality Control", "location": "Regional Hub",     "date": (base_date + timedelta(days=2)).strftime('%Y-%m-%d'), "verified": True}
    ]

    if status == 'FLAGGED':
        journey.extend([
            {"stage": "Transit",      "location": "Unknown Checkpoint",    "date": (base_date + timedelta(days=5)).strftime('%Y-%m-%d'), "verified": False},
            {"stage": "Distribution", "location": "Unregistered Warehouse","date": (base_date + timedelta(days=8)).strftime('%Y-%m-%d'), "verified": False}
        ])
    else:
        journey.extend([
            {"stage": "Transit",      "location": "Verified Checkpoint","date": (base_date + timedelta(days=5)).strftime('%Y-%m-%d'), "verified": True},
            {"stage": "Distribution", "location": "Official Retailer",  "date": (base_date + timedelta(days=8)).strftime('%Y-%m-%d'), "verified": True}
        ])

    return journey

# ─── Reasoning-first audit report ────────────────────────────────────────────
...
Your task:
1. State the verification STATUS clearly (VERIFIED or FLAGGED)
2. Explain the SPECIFIC REASON for flagging — what exactly looks wrong?
...
Provide a brief 2-paragraph executive summary of the risk patterns. Focus on which manufacturers are the biggest liability and what this likely indicates operationally."""
        ...
        return response.choices[0].message.content, patterns_dict
...
# ─── Conversational chat about a product ─────────────────────────────────────
...
Product record (includes all available fields, even custom ones from the original data source):
{data}

Chain of custody: {journey}
Unverified stages: {unverified_stages if unverified_stages else "None — all stages verified"}

Answer the user's questions about this product clearly and specifically.
If a field is available in the product record above, use it to answer.
